...
import tests, { type TestDefinition } from './tests/definitions.js';
...
async function run() {
  ...
  let selected: TestDefinition[] = tests;
  ...
  let passed = 0;
  let failed = 0;
  const resultsByMethod = new Map<string, { passed: string[]; failed: { name: string; reason: string }[] }>();

  const trackResult = (method: string, status: 'passed' | 'failed', name: string, reason?: string) => {
  ...
  for (const test of ordered) {
    ...
    try {
      ...
      if (failures.length === 0) {
        ...
        failed += 1;
        const reason = failures.map((f: any) => f.error).join('; ');
