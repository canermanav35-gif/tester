...
async function getStaffPair() {
  ...
  const secondary = active.find((s: any) => s.id !== primary?.id) || list.find((s: any) => s.id !== primary?.id);
...
export async function prepareAdminLeaveCreate(test: any) {
...
export async function prepareLeaveUpdate(test: any) {
  const { staffID, substituteID } = await getStaffPair();
...
export async function prepareLeaveDelete(test: any) {
  const fixtures = getFixtures();
...
export function prepareLocationCreate(test: any) {
  const name = test.data?.name && test.data.name.trim() ? test.data.name : uniqueName();
  const rooms =
    Array.isArray(test.data?.rooms) && test.data.rooms.length > 0
      ? test.data.rooms
...
export async function prepareLocationDetail(test: any) {
  const fixtures = getFixtures();
...
export async function prepareLocationUpdate(test: any) {
  const fixtures = getFixtures();
  let locationId = fixtures.location_id;
  if (!locationId) {
    const res = await send({ method: 'GET', url: '/api/locations', user: 'admin' });
    locationId = res?.data?.data?.[0]?.id;
    if (!locationId) throw new Error('No location id found to update');
...
export async function prepareLocationDelete(test: any) {
  const fixtures = getFixtures();
  let locationId = fixtures.location_id;
  if (!locationId) {
    const res = await send({ method: 'GET', url: '/api/locations', user: 'admin' });
    locationId = res?.data?.data?.[0]?.id;
    if (!locationId) throw new Error('No location id found to delete');
    setFixture('location_id', locationId);
  }
...
export async function prepareLocationRooms(test: any) {
  const fixtures = getFixtures();
  let locationId = fixtures.location_id;
  if (!locationId) {
    const res = await send({ method: 'GET', url: '/api/locations', user: 'admin' });
    locationId = res?.data?.data?.[0]?.id;
    if (!locationId) throw new Error('No location id found to fetch rooms');
    setFixture('location_id', locationId);
  }
