...
const normalizeUserKey = (user) => (user ? String(user).trim().toUpperCase().replace(/[^A-Z0-9]/g, '_') : '');
const tokenCache = new Map();
const baseURL = process.env.API_BASE_URL;
function getEnvToken(userKey) {
...
async function loginAndGetToken(user, userKey) {
    if (!baseURL) {
        throw new Error('Missing API_BASE_URL in environment for login');
    }
    const creds = getCredentials(userKey);
    if (!creds) {
        throw new Error(`Missing credentials for user "${user}". Set ${userKey}_DOMAIN_ID, ${userKey}_IDENTIFIER, and ${userKey}_PASSWORD.`);
    }
